...
int main() {
  ...
  for (int i = 0; i < 40; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      A_pont_std[i][j] = A[i][j] / std_pont[i];
    }
    b_pont_std[i] = b[i] / std_pont[i];
  }
  ...
  VecDoub std_fil = A_fil * x_fil - b_fil;

  for (int i = 0; i < 82; i++) {
    for (int j = 0; j < 11; j++) {
      A_fil_std[i][j] = A_fil[i][j] / std_fil[i];
    }
    b_fil_std[i] = b_fil[i] / std_fil[i];
  }
